...
public class SpmCw extends SpmState{
	
	private static SpmCw spmCw = new SpmCw();
	...
	@Override
	public CommonMessageFrameImpl doRawConvertor(SpmContext context, CommonMessageFrameImpl cmfi) {
		...
		try {
			// 변경 예정
			String msgLog = "realTx : ";
			for(i=0; i < subLength*2; i+=8){
				realTx.add((double)cci.getFloat(cmfi.subString.substring(i, i+8)));
				msgLog += cci.getFloat(cmfi.subString.substring(i, i+8)) + ", ";
			}
			// 시간축 개수
			for(j=0; j < realTx.size(); j++){
				realT.add((double)j);
			}
	...
	public CommonMessageFrameImpl doFftConvertor(SpmContext context,
		...
		try {
			...
			for(int i=0, j=0; i < (subDataLength*2); i+=8, j++){
				...
				/*msgLog +=((double)(j+1)/(subDataLength/4)-0.5)*fs + ", ";
				msgLog2 += cci.getFloat(cmfi.subString.substring(i, i+8)) + ", ";*/
			}
		} catch (Exception e) {
	...
	@Override
	public CommonMessageFrameImpl doDetConvertor(SpmContext context, CommonMessageFrameImpl cmfi) {
		int i = 0;
		...
		for(i=0; i < pointCtn*16; i+=16){
			...
			float speedFloat = cci.getFloat(subDataString.substring(i+16, i+24));
